...
uint16_t sequence_1_P_size = sizeof(sequence_2_P) / sizeof(sequence_2_P[0]);
...
void update_sequence(uint32_t current_sequence)
{
    ...
    internal_sequence_timer = get_millis();
}
...
void textile_1_loop(void){
    ...
    {
        uint32_t tmp = SINGLE;
        write_data_table(TAG_TEXTILE_1_TRIGGER_TYPE, tmp, sizeof(tmp));
    ...
    if( textile_1_current_sequence == SEQUENCE_A ){
        uint32_t current_timer = get_millis() - internal_sequence_timer;
        for (uint16_t i = 0 ; i < sequence_1_A_size ; i++)
        {
               ...
               (current_timer <= sequence_1_A[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    ...
                    uint8_t mapped_value = map(0 , duration_left, sequence_1_A[i].value_starts_at, sequence_1_A[i].value_ends_at);
                    setPin(sequence_1_A[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_1_A[i].value_starts_at, sequence_1_A[i].value_ends_at);
                    setPin(sequence_1_A[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_1_A[sequence_1_A_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_1_A_size ; i++)
            {
    ...
    if( textile_1_current_sequence == SEQUENCE_B ){
        ...
        for (uint16_t i = 0 ; i < sequence_1_B_size ; i++)
        {
               ...
               (current_timer <= sequence_1_B[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_1_B[i].value_starts_at, sequence_1_B[i].value_ends_at);
                    setPin(sequence_1_B[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_1_B[i].value_starts_at, sequence_1_B[i].value_ends_at);
                    setPin(sequence_1_B[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_1_B[sequence_1_B_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_1_B_size ; i++)
            {
    ...
    if( textile_1_current_sequence == SEQUENCE_C ){
        ...
        for (uint16_t i = 0 ; i < sequence_1_C_size ; i++)
        {
               ...
               (current_timer <= sequence_1_C[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_1_C[i].value_starts_at, sequence_1_C[i].value_ends_at);
                    setPin(sequence_1_C[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_1_C[i].value_starts_at, sequence_1_C[i].value_ends_at);
                    setPin(sequence_1_C[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_1_C[sequence_1_C_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_1_C_size ; i++)
            {
    ...
    if( textile_1_current_sequence == SEQUENCE_D ){
        ...
        for (uint16_t i = 0 ; i < sequence_1_D_size ; i++)
        {
               ...
               (current_timer <= sequence_1_D[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_1_D[i].value_starts_at, sequence_1_D[i].value_ends_at);
                    setPin(sequence_1_D[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_1_D[i].value_starts_at, sequence_1_D[i].value_ends_at);
                    setPin(sequence_1_D[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_1_D[sequence_1_D_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_1_D_size ; i++)
            {
    ...
    if( textile_1_current_sequence == SEQUENCE_P ){
        ...
        for (uint16_t i = 0 ; i < sequence_1_P_size ; i++)
        {
               ...
               (current_timer <= sequence_1_P[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_1_P[i].value_starts_at, sequence_1_P[i].value_ends_at);
                    setPin(sequence_1_P[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_1_P[i].value_starts_at, sequence_1_P[i].value_ends_at);
                    setPin(sequence_1_P[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_1_P[sequence_1_P_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_1_P_size ; i++)
            {
    ...
    if( textile_2_current_sequence == SEQUENCE_A ){
        ...
        for (uint16_t i = 0 ; i < sequence_2_A_size ; i++)
        {
               ...
               (current_timer <= sequence_2_A[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_2_A[i].value_starts_at, sequence_2_A[i].value_ends_at);
                    setPin(sequence_2_A[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_2_A[i].value_starts_at, sequence_2_A[i].value_ends_at);
                    setPin(sequence_2_A[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_2_A[sequence_2_A_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_2_A_size ; i++)
            {
    ...
    if( textile_2_current_sequence == SEQUENCE_B ){
        ...
        for (uint16_t i = 0 ; i < sequence_2_B_size ; i++)
        {
               ...
               (current_timer <= sequence_2_B[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_2_B[i].value_starts_at, sequence_2_B[i].value_ends_at);
                    setPin(sequence_2_B[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_2_B[i].value_starts_at, sequence_2_B[i].value_ends_at);
                    setPin(sequence_2_B[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_2_B[sequence_2_B_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_2_B_size ; i++)
            {
    ...
    if( textile_2_current_sequence == SEQUENCE_C ){
        ...
        for (uint16_t i = 0 ; i < sequence_2_C_size ; i++)
        {
               ...
               (current_timer <= sequence_2_C[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_2_C[i].value_starts_at, sequence_2_C[i].value_ends_at);
                    setPin(sequence_2_C[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_2_C[i].value_starts_at, sequence_2_C[i].value_ends_at);
                    setPin(sequence_2_C[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_2_C[sequence_2_C_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_2_C_size ; i++)
            {
    ...
    if( textile_2_current_sequence == SEQUENCE_D ){
        ...
        for (uint16_t i = 0 ; i < sequence_2_D_size ; i++)
        {
               ...
               (current_timer <= sequence_2_D[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_2_D[i].value_starts_at, sequence_2_D[i].value_ends_at);
                    setPin(sequence_2_D[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_2_D[i].value_starts_at, sequence_2_D[i].value_ends_at);
                    setPin(sequence_2_D[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_2_D[sequence_2_D_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_2_D_size ; i++)
            {
    ...
    if( textile_2_current_sequence == SEQUENCE_P ){
        ...
        for (uint16_t i = 0 ; i < sequence_2_P_size ; i++)
        {
               ...
               (current_timer <= sequence_2_P[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_2_P[i].value_starts_at, sequence_2_P[i].value_ends_at);
                    setPin(sequence_2_P[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_2_P[i].value_starts_at, sequence_2_P[i].value_ends_at);
                    setPin(sequence_2_P[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_2_P[sequence_2_P_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_2_P_size ; i++)
            {
    ...
    if( textile_3_current_sequence == SEQUENCE_A ){
        ...
        for (uint16_t i = 0 ; i < sequence_3_A_size ; i++)
        {
               ...
               (current_timer <= sequence_3_A[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_3_A[i].value_starts_at, sequence_3_A[i].value_ends_at);
                    setPin(sequence_3_A[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_3_A[i].value_starts_at, sequence_3_A[i].value_ends_at);
                    setPin(sequence_3_A[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_3_A[sequence_3_A_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_3_A_size ; i++)
            {
    ...
    if( textile_3_current_sequence == SEQUENCE_B ){
        ...
        for (uint16_t i = 0 ; i < sequence_3_B_size ; i++)
        {
               ...
               (current_timer <= sequence_3_B[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_3_B[i].value_starts_at, sequence_3_B[i].value_ends_at);
                    setPin(sequence_3_B[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_3_B[i].value_starts_at, sequence_3_B[i].value_ends_at);
                    setPin(sequence_3_B[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_3_B[sequence_3_B_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_3_B_size ; i++)
            {
    ...
    if( textile_3_current_sequence == SEQUENCE_C ){
        ...
        for (uint16_t i = 0 ; i < sequence_3_C_size ; i++)
        {
               ...
               (current_timer <= sequence_3_C[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_3_C[i].value_starts_at, sequence_3_C[i].value_ends_at);
                    setPin(sequence_3_C[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_3_C[i].value_starts_at, sequence_3_C[i].value_ends_at);
                    setPin(sequence_3_C[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_3_C[sequence_3_C_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_3_C_size ; i++)
            {
    ...
    if( textile_3_current_sequence == SEQUENCE_D ){
        ...
        for (uint16_t i = 0 ; i < sequence_3_D_size ; i++)
        {
               ...
               (current_timer <= sequence_3_D[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_3_D[i].value_starts_at, sequence_3_D[i].value_ends_at);
                    setPin(sequence_3_D[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_3_D[i].value_starts_at, sequence_3_D[i].value_ends_at);
                    setPin(sequence_3_D[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_3_D[sequence_3_D_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_3_D_size ; i++)
            {
    ...
    if( textile_3_current_sequence == SEQUENCE_P ){
        ...
        for (uint16_t i = 0 ; i < sequence_3_P_size ; i++)
        {
               ...
               (current_timer <= sequence_3_P[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_3_P[i].value_starts_at, sequence_3_P[i].value_ends_at);
                    setPin(sequence_3_P[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_3_P[i].value_starts_at, sequence_3_P[i].value_ends_at);
                    setPin(sequence_3_P[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_3_P[sequence_3_P_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_3_P_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_A ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_A_size ; i++)
        {
               ...
               (current_timer <= sequence_4_A[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_A[i].value_starts_at, sequence_4_A[i].value_ends_at);
                    setPin(sequence_4_A[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_A[i].value_starts_at, sequence_4_A[i].value_ends_at);
                    setPin(sequence_4_A[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_A[sequence_4_A_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_A_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_B ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_B_size ; i++)
        {
               ...
               (current_timer <= sequence_4_B[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_B[i].value_starts_at, sequence_4_B[i].value_ends_at);
                    setPin(sequence_4_B[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_B[i].value_starts_at, sequence_4_B[i].value_ends_at);
                    setPin(sequence_4_B[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_B[sequence_4_B_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_B_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_C ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_C_size ; i++)
        {
               ...
               (current_timer <= sequence_4_C[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_C[i].value_starts_at, sequence_4_C[i].value_ends_at);
                    setPin(sequence_4_C[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_C[i].value_starts_at, sequence_4_C[i].value_ends_at);
                    setPin(sequence_4_C[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_C[sequence_4_C_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_C_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_D ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_D_size ; i++)
        {
               ...
               (current_timer <= sequence_4_D[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_D[i].value_starts_at, sequence_4_D[i].value_ends_at);
                    setPin(sequence_4_D[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_D[i].value_starts_at, sequence_4_D[i].value_ends_at);
                    setPin(sequence_4_D[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_D[sequence_4_D_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_D_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_E ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_E_size ; i++)
        {
               ...
               (current_timer <= sequence_4_E[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_E[i].value_starts_at, sequence_4_E[i].value_ends_at);
                    setPin(sequence_4_E[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_E[i].value_starts_at, sequence_4_E[i].value_ends_at);
                    setPin(sequence_4_E[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_E[sequence_4_E_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_E_size ; i++)
            {
    ...
    if( textile_4_current_sequence == SEQUENCE_P ){
        ...
        for (uint16_t i = 0 ; i < sequence_4_P_size ; i++)
        {
               ...
               (current_timer <= sequence_4_P[i].time_ends_at))
            {
                ...
                if(current_timer <= middle_point){
                    uint32_t duration_left = middle_point - current_timer;
                    uint8_t mapped_value = map(0 , duration_left, sequence_4_P[i].value_starts_at, sequence_4_P[i].value_ends_at);
                    setPin(sequence_4_P[i].led_index.negative_pin, mapped_value);
                }
                if(current_timer > middle_point){
                    uint32_t duration_left = current_timer - middle_point;
                    uint8_t mapped_value = map(duration_left , 0, sequence_4_P[i].value_starts_at, sequence_4_P[i].value_ends_at);
                    setPin(sequence_4_P[i].led_index.negative_pin, mapped_value);
                }
            }
        }
        if(current_timer > sequence_4_P[sequence_4_P_size].time_ends_at)
        {
            for (uint16_t i = 0 ; i < sequence_4_P_size ; i++)
            {
